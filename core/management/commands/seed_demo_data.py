...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        enable_demo_passwords = os.environ.get('ENABLE_DEMO_PASSWORDS', 'False').lower() == 'true'
        create_demo_admin = os.environ.get('CREATE_DEMO_ADMIN', 'False').lower() == 'true'

        readers = []
        for username, email in reader_specs:
            user, _ = User.objects.get_or_create(username=username, defaults={'email': email})
            if enable_demo_passwords and not user.has_usable_password():
                user.set_password('BookVibeReader123!')
                user.save()
            elif not enable_demo_passwords and user.has_usable_password():
